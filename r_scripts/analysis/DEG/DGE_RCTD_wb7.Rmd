---
title: "Differential Gene Expression without blocco7"
...
res_path = "/mnt/europa/valerio/repositories/cachetic_visiumHD/results/DGE/DGE_pb_wb7/"
```

```{r}
load("/mnt/europa/valerio/data/Rdata/pseudobulk_celltype_wb7.RData")
```
...
# save(contrast_list, file = paste0(Rd_path, "contrast_list.RData"))
...
deg_fun_emma <- \(pseudobulk_exp, contrast_list)
{
  ...
  for (contrast_name in names(contrast_list)) {
    ...
    for (cell_type in unique(pseudobulk_exp$cell_type)) {
      ...
      y <- DGEList(assay(ct_pb), samples = as.data.frame(colData(ct_pb)))
...
save(degged, file = paste0(Rd_path, "deg_results_list.RData"))
