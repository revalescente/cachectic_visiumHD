...
if all_samples_processed:
    print(f"  [SKIP] All samples for {BLOCCO_KEY} already exist in {save_dir}. Exiting.")
    sys.exit(0)
...
for sample_name, sample_info in block_samples.items():
    ...
    try:
        all_poly = gpd.read_file(f"{intissue_gfp_dir}/{BLOCCO_KEY}_{sample_name}.geojson")
    ...
    try:
        ...
        print(f"  [ERROR] Processing Arivis/Feature extraction failed for {sample_name}: {e}")
        continue
