...
def morphological_filtering(sdata, filters):
    ...
    if not match:
        ...
        raise ValueError(
            f"Could not parse blocco_key and samples_key from: {sdata.path}"
        )
    ...
    sdata["nuclei_counts_nop"].obs = sdata["nuclei_counts_nop"].obs.join(
        features_df[cols], how="left"
    )
...
def features_extraction(sdata, nuclei_element_name="nuclei_boundaries"):
    ...
    element_extent = sd.get_extent(
        sdata[nuclei_element_name], coordinate_system=blocco_key, exact=True
    )
    ...
    try:
        ...
        print(
            "Warning: Could not find 'label_index_to_category' mapping. Index will not be set to cell_id."
        )
    return props_df
...
def find_extreme_observations(
    ...
    try:
        ...
        raise KeyError(
            f"Table with key '{table_key}' not found in the SpatialData object."
        )
    ...
    for column in feature_columns:
        if column not in obs_df.columns:
            print(
                f"⚠️ Warning: Column '{column}' not found in '{table_key}.obs'. Skipping."
            )
            continue
    ...
    display_cols = (
        ["feature", "extreme_type", id_col_name] + feature_columns + all_original_cols
    )
...
def sopa_attrs_check(sdata):
    ...
    if attrs["bins_table"] != "filtered":
        print(
            f"Attribute 'bins_table' has value '{attrs['bins_table']}', expected 'filtered'"
        )
        return False
    ...
    if not cell_seg_image.endswith(expected_suffix):
        print(
            f"Attribute 'cell_segmentation_image' does not end with '{expected_suffix}'"
        )
        return False
    ...
    if "boundaries_shapes" in attrs and not attrs["boundaries_shapes"].startswith(
        blocco
    ):
        print(
            f"Warning: 'boundaries_shapes' value '{attrs['boundaries_shapes']}' doesn't match expected blocco '{blocco}'"
        )
    ...
    if (
        "tissue_segmentation_image" in attrs
        and attrs["tissue_segmentation_image"] != cell_seg_image
    ):
        print(
            f"Warning: 'tissue_segmentation_image' value '{attrs['tissue_segmentation_image']}' doesn't match 'cell_segmentation_image'"
        )
        # Not returning False here as you mentioned only checking the two main attributes
...
def segmentation_step(sdata):
    ...
    if match:
        ...
        raise ValueError(
            f"Could not parse blocco_key and samples_key from: {sdata_path}"
        )
    ...
    sopa.segmentation.stardist(
        ...
        image_key=sdata.attrs["cell_segmentation_image"],
        min_area=10,
        delete_cache=True,
        recover=False,
        prob_thresh=0.2,
        nms_thresh=0.6,
        key_added=f"nuclei_boundaries",
    )
...
def postprocess_step(
    ...
    **kwargs,
    ...
    sopa.aggregate(
        sdata,
        key_added="nuclei_counts_nop",
        bins_key="filtered",
        shapes_key="nuclei_boundaries",
    ...
    if filters is None:
        filters = {
            "area": (50, 4000),  # filter out the bigger and smaller nuclei
            "eccentricity": (
                None,
                0.95,
            ),  # filter the most parabolic nuclei (one edge almost straight)
            "solidity": (0.7, None),  # filter the most concave
            "extent": (0.2, None),
        }  # filter the most irregular nuclei (mix between parabolic and concave)
